...
function shortLine(blameInfo) {
  const name = blameInfo.author;
  const time = distanceInWordsToNow(new Date(blameInfo.authorTime * 1000));
  const summary = blameInfo.summary;

  return `${name}, ${time} ago - ${summary}`;
}
